...
1. 首选 ROW 格式：
   ...
   理由：主从复制一致性最高，兼容性强，避免业务复杂后出现数据不一致问题 —— 日志体积大的问题可通过「定时清理 binlog」「开启 binlog 压缩」解决。
...
2. 配合关键配置：
   ...
   （redo log 提交时强制刷盘），实现「双一配置」，确保主从复制和数据恢复的可靠性。

### 面试总结（直接背）

MySQL binlog 有三种格式，核心结论：

1. STATEMENT：记录 SQL 语句，体积小但一致性差，几乎不用；
2. ROW：记录行变更，一致性高但体积大，生产环境首选；
3. MIXED：自动切换前两种格式，平衡体积和一致性，但排查复杂，过渡使用。
