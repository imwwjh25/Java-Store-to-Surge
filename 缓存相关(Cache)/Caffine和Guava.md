Caffeine 是基于 Guava Cache 设计的新一代高性能缓存库，核心目标是解决 Guava Cache 在**性能、命中率、并发控制、内存管理**等方面的核心不足，同时兼容 Guava 的 API 以降低迁移成本。以下是 Caffeine 对 Guava Cache 的关键改进点，结合原理和场景对比说明：
...
- 场景 ：缓存中有 3 个数据 A（高频访问但最近 10s 未访问）、B（低频但最近访问）、C（极低频），缓存满时需淘汰一个。
